...
//    property string thema: setting.thema //25294A
    ...
    Highlight {
        ...
        mm: {
            if(tmp_mm === "January") return "01"
            if(tmp_mm === "February") return "02"
            if(tmp_mm === "March") return "03"
            if(tmp_mm === "April") return "04"
            if(tmp_mm === "May") return "05"
            if(tmp_mm === "June") return "06"
            if(tmp_mm === "July") return "07"
            if(tmp_mm === "August") return "08"
            if(tmp_mm === "September") return "09"
            if(tmp_mm === "October") return "10"
            if(tmp_mm === "November") return "11"
            if(tmp_mm === "December") return "12"
